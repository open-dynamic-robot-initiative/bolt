 ...
 * Gesellschaft
 *
 * @brief Bolt humanoid low level drivers.
 */
...
public:
     ...
     */
    const Eigen::Ref<const Eigen::Vector9d> get_joint_positions()
    {
     ...
     * prior to any getter to have up to date data.
     */
    const Eigen::Ref<const Eigen::Vector9d> get_joint_velocities()
    {
     ...
     * prior to any getter to have up to date data.
     */
    const Eigen::Ref<const Eigen::Vector9d> get_joint_torques()
    {
     ...
     */
    const Eigen::Ref<const Eigen::Vector9d> get_joint_target_torques()
    {
     ...
     */
    const Eigen::Ref<const Eigen::Matrix<bool, BOLT_HUMANOID_NB_MOTOR, 1> >
    get_motor_enabled()
    {
     ...
     * cards).
     */
    const Eigen::Ref<const Eigen::Matrix<int, BOLT_HUMANOID_NB_MOTOR_BOARD, 1> >
     ...
     *
     * @return Eigen::Ref<Eigen::Matrix<double, BOLT_HUMANOID_NB_SLIDER, 1> >
     */
    const Eigen::Ref<const Eigen::Matrix<double, BOLT_HUMANOID_NB_SLIDER, 1> >
...
private:
    ...
    Eigen::Matrix<bool, BOLT_HUMANOID_NB_MOTOR, 1> motor_enabled_;

    /** @brief Motor readiness to receive commands (ready/not ready). */
    Eigen::Matrix<bool, BOLT_HUMANOID_NB_MOTOR, 1> motor_ready_;

    /** @brief Motor Board status (enabled/disabled). */
    Eigen::Matrix<bool, BOLT_HUMANOID_NB_MOTOR_BOARD, 1> motor_board_enabled_;

    /** @brief Motor Board Error code (int).
     */
    Eigen::Matrix<int, BOLT_HUMANOID_NB_MOTOR_BOARD, 1> motor_board_errors_;
    ...
    Eigen::Vector9d joint_positions_;

    /** @brief Joint velocities. */
    Eigen::Vector9d joint_velocities_;

    /** @brief Joint torques. */
    Eigen::Vector9d joint_torques_;

    /** @brief Target joint torques, reference from the controller. */
    Eigen::Vector9d joint_target_torques_;
    ...
    Eigen::Matrix<double, BOLT_HUMANOID_NB_SLIDER, 1> slider_positions_;
